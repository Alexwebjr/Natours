...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  const url = `${req.protocol}://${req.get('host')}/me`; //built url
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email }).select('+password'); //toSelect hiding field password
...
exports.isLoggedIn = async (req, res, next) => {
  ...
  if (req.cookies.jwt) {
    try {
      ...
      res.locals.user = currentUser; //passing data to template
      return next();
    } catch (err) {
      return next();
    }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findById(req.user.id).select('+password'); //toExplicit get the password
